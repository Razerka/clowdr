...
export default function SUPermissionGrants(): JSX.Element {
    ...
    const [insertM, insertResponse] = useInsertSuPermissionGrantMutation();
    const insert = useMemo<Insert<SuPermissionGrantFragment>>(
        () => ({
            ongoing: insertResponse.loading,
            generateDefaults: () => ({
                id: uuidv4(),
                userId: currentUser.user.id,
            }),
        ...
        [currentUser.user.id, insertM, insertResponse.loading]
    );

    const [deleteM, deleteResponse] = useDeleteSuPermissionGrantsMutation();
    const deleteO = useMemo<Delete<SuPermissionGrantFragment>>(
        () => ({
            ongoing: deleteResponse.loading,
            start: (keys) => {
                deleteM({
                    variables: {
                        ids: keys,
        ...
        [deleteM, deleteResponse.loading]
    );
    ...
    const [initialiseRoleForUser, initialiseRoleForUserResponse] = useInitialiseRoleForUserMutation();
    return (
        <>
            <Menu>
                <MenuButton as={Button} colorScheme="purple" isLoading={initialiseRoleForUserResponse.loading}>
            ...
            <CRUDTable
                data={
                    !allPermissionsResponse.loading &&
